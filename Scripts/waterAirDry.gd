...
@export var erase_strength: float = 0.5   # Amount per second applied to target
...
var over_target: Node = null              # Target currently under the eraser
...
func _process(delta: float) -> void:
	# Apply erase effect while dragging over target
	if is_dragging and over_target and over_target.is_inside_tree():
		if over_target.has_method("apply_erase_fade"):
...
func _on_area_entered(area: Area2D) -> void:
	...
	if parent == self:
		return
	# Only interact with nodes that implement apply_erase_fade
